...
function processInput() {
  ...
  for (const line of lines) {
    ...
    if (x2 < x1 || y2 < y1 || z2 < z1) {
      console.log('ERROR: not expecting inverted coordinates');
      return null;
    }
...
function settleBrick(master, floor) {
  if (master.z1 != floor) {
    return;
  } // floor does not match the bottom of the (tower) brick
  ...
  while (true) {
    ...
    if (level == undefined) {
      bringBrickDown(master, 1);
      return;
    } // no holder found

    for (const brick of level) {
      if (!bricksMatch(master, brick)) {
        continue;
      }
    ...
    if (master.z1 != newFloor) {
      bringBrickDown(master, newFloor);
    }
